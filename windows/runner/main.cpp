                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"TuTu4kA VPN"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"TuTu4kAVPNMutex");
  HWND handle = FindWindowA(NULL, "TuTu4kA VPN");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"TuTu4kA VPN"))
    {
      return false;
    }
  ...
  if (!window.Create(L"TuTu4kA VPN", origin, size))
  {
    return EXIT_FAILURE;
  }
