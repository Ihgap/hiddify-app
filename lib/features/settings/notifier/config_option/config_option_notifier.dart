    ...
    ref.listen(ConfigOptions.singboxConfigOptions, (previous, next) async {
      ...
      if (next != previous && next != serviceSingboxOptions) {
        if (_lastUpdate == null || DateTime.now().difference(_lastUpdate!) > const Duration(milliseconds: 100)) {
          ...
          // Полный перезапуск (off→on), а не reconnect(): «лёгкий» reconnect на
          // некоторых Android падает с "configure tun interface: permission
          // denied" при переконфигурации tun на лету. Off→on переинициализирует
          // VpnService и применяет новый конфиг надёжно.
          loggy.debug("config option changed, restarting connection");
